...
module.exports = class UserService extends Service {
    ...
    getUserById(id) {
        ...
        return user.select('id', 'firstName', 'lastName', 'username', 'mail')
    }

    async deleteUserById(userId) {
        const { User } = this.server.models();

        const userToDelete = await User.query().findById(userId);
    ...
    async login(mail, password) {
        ...
        try {
            ...
            return { authenticated: true,role: user.role, message: 'successful' };
